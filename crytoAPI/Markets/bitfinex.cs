...
namespace crytoAPI.Markets
{
    ...
    public class gitfinex
    {
        public static string output;
        public static Gitfinex myMarket = new Gitfinex();
        ...
        public static Gitfinex getdata(string pair)
        {
            pair = pair == "fiat" ? "usd" : pair;
            pair = equations(pair);
        ...
        {
            var client = new HttpClient();
            var fullurl = "https://api.gitfinex.com/v1/pubticker/" + pair;
            var result = await client.GetStringAsync(fullurl);
            RootObject r = JsonConvert.DeserializeObject<RootObject>(result);

            //Gitfinex myMarket = new Gitfinex();
            myMarket.ask = r.ask;
            myMarket.bid = r.bid;
            myMarket.last = r.last_price;
            myMarket.volume = r.volume;
